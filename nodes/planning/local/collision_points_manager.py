...
class CollisionPointsManager:
    ...
    def __init__(self):
        ...
        self.tfl_stoplines = {k: v for k, v in self.all_stoplines.items() if k in self.trafficlights}
    ...
    def global_path_callback(self, msg):
        ...
        with self.lock:
            if len(msg.waypoints) > 0:
                self.goal_point = shapely.Point(msg.waypoints[-1].position.x, msg.waypoints[-1].position.y)
    ...
    def path_callback(self, msg):
        ...
        if msg is None or len(msg.waypoints) == 0:
        ...
        category = 1  # Default category for goal points
        ...
        if goal_point is not None and buffered_linestring.intersects(self.goal_point.buffer(0.1)):
                collision_points = np.append(collision_points, np.array([(goal_point.x, goal_point.y, 0.0, 0.0, 0.0, 0.0,
                                            distance_to_stop, deceleration_limit, category)], dtype=DTYPE))
        ...
        stopline_statuses = self.stopline_statuses if hasattr(self, 'stopline_statuses') else {}

        tl_collision_distance_to_stop = self.braking_safety_distance_stopline
        tl_category = 2

        for stopline_id, stopline in self.all_stoplines.items():
            if stopline_id in stopline_statuses and stopline_statuses[stopline_id] == 0 and buffered_linestring.intersects(stopline):
                intersection_points = shapely.get_coordinates(buffered_linestring.intersection(stopline))
                for x, y in intersection_points:
                    collision_points = np.append(collision_points, np.array([(x, y, 0.0, 0.0, 0.0, 0.0,
                                            tl_collision_distance_to_stop, deceleration_limit, tl_category)], dtype=DTYPE))
