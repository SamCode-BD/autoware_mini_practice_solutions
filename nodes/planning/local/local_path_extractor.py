...
class LocalPathExtractor:
   ...
   def extract_local_path(self, _):
       try:
           ...
           # Compute cumulative Euclidean distances along the global path
           distances = np.linalg.norm(np.diff(global_path_xyz[:, :2], axis = 0), axis=1)
           global_path_distances = np.cumsum(distances)
