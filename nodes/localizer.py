...
class Localizer:
    ...
    def transform_coordinates(self, msg):
        number1, number2 = self.transformer.transform(msg.latitude, msg.longitude)
        final_x = number1 - self.origin_x
        final_y = number2 - self.origin_y
        ...
        current_pose_msg.pose.position.x = final_x
        current_pose_msg.pose.position.y = final_y
        current_pose_msg.pose.position.z = final_z
        current_pose_msg.pose.orientation = orientation
        self.current_pose_pub.publish(current_pose_msg)

        #velocity message
        velocity = math.sqrt((msg.north_velocity*msg.north_velocity) + (msg.east_velocity*msg.east_velocity))
