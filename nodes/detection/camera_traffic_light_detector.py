...
class CameraTrafficLightDetector:
    ...
    def local_path_callback(self, local_path_msg):
        ...
        if len(local_path_msg.waypoints) > 0:
            local_path_linestring = shapely.LineString([(waypoint.position.x, waypoint.position.y, waypoint.position.z) for waypoint in local_path_msg.waypoints])
    ...
    def camera_image_callback(self, camera_image_msg):
        ...
        trafficLightResultArray.header.stamp = camera_image_msg.header.stamp
        rois = []
        classes = []
        scores = []
        ...
        if len(stoplines_on_path) > 0:
            #use the already created transform listener to extract the transform
            try:
                ...
                rospy.logwarn("%s - Could not get transform from %s to %s: %s", rospy.get_name(), transform_from_frame, camera_image_msg.header.frame_id, str(e))
                return
            ...
            if len(rois) > 0:
                ...
                for cl, (stoplineId, plId, _, _, _, _) in zip(classes, rois):
                    ...
                    trafficLightResultArray.results.append(tfl_result)
        

        self.tfl_status_pub.publish(trafficLightResultArray)
        ...
        for linkId in stoplines_on_path:
            for plId, traffic_lights in self.trafficlights[linkId].items():
                ...
                for x, y, z in traffic_lights.values():
                    ...
                    # if there are not enough points, we cannot create a roi
                    if u < 0 or u >= self.camera_model.width or v < 0 or v >= self.camera_model.height:
                        rospy.logwarn_throttle(10, "%s - Traffic light %s is not visible in image, skipping", rospy.get_name(), plId)
                        continue
    ...
    def create_roi_images(self, image, rois):
        ...
        for _, _, min_u, max_u, min_v, max_v in rois:
            ...
            roi_images.append(roi.astype(np.float32))
